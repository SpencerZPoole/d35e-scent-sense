...
| Familiar odor identification | Not yet automated | Planned for `v0.6.0`. |
| Water and water-breathing UI | Helper only | Rule helper exists; GM-facing controls are planned for `v0.6.0` and `v0.7.0`. |
| False/powerful odor sources | Helper only | Current masking flag is simple; source modeling is planned for `v0.6.0`. |
...
- GM tools cover scene and token scent context without editing actor flags unexpectedly.
